    ...
    public void onClick(View view) {
        if (view.getId() != R.id.left_arrow && view.getId() != R.id.right_arrow && view.getId() != R.id.del && view.getId() != R.id.go2cal) {
            if(cnt < 4){
                ...
                Toast.makeText(GameActivity.this, "你已经选择了4张扑克了，请勿再次选择", Toast.LENGTH_LONG).show();
            }
        }
        else {
            int sz = fill.size();
            if(view.getId() == R.id.del){
                if(sz == 0){
                    Toast.makeText(GameActivity.this, "你还未选择扑克，请选择", Toast.LENGTH_LONG).show();
                }else{
                ...
                Intent intent = new Intent(GameActivity.this, CalculateActivity.class);
                intent.putExtra("Data", (Serializable) trans);
                startActivity(intent);
            }else if(view.getId() == R.id.go2cal && sz != 4){
                Toast.makeText(GameActivity.this, "你的扑克数量还未到4，请继续选择扑克", Toast.LENGTH_LONG).show();
            }
        }
